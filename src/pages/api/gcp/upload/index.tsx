...
const gcpConfig = JSON.parse(
  Buffer.from(gcpConfigBase64!, "base64").toString("utf-8"),
);
...
router.post(async (req, res) => {
  ...
  if (!recipients || recipients.trim().length === 0) {
    return res
      ...
      .json({ data: null, error: [{ message: "Invalid recipients" }] });
  }
  ...
  await file.save(
    ...
    (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ data: null, error: [{ message: "Error saving file" }] });
      }
    },
  );
